...
- (void)viewWillAppear:(BOOL)animated {
    ...
    // 3
    [_fromMap setRegion:viewRegion animated:YES];
}
...
- (void)forwardGeocodingDidSucceed:(BSForwardGeocoder *)geocoder withResults:(NSArray *)results
{
    ...
    for (int i = 0, resultCount = [results count]; i < resultCount; i++) {
        ...
        [self.fromMap addAnnotation:placemark];
    }
    ...
    if ([results count] == 1) {
        ...
        [self.fromMap setRegion:place.coordinateRegion animated:YES];
    }
    
    // Dismiss the keyboard
    [self.searchFrom resignFirstResponder];
}
...
{
	NSLog(@"Searching for: %@", self.searchFrom.text);
    ...
    [self.forwardGeocoder forwardGeocodeWithQuery:self.searchFrom.text regionBiasing:nil viewportBiasing:bounds success:^(NSArray *results) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath
	...
	if([[change valueForKey:@"new"] intValue] == 1 && [action isEqualToString:@"GMAP_ANNOTATION_SELECTED"])  {
		if ([((MKAnnotationView*) object).annotation isKindOfClass:[CustomPlacemark class]]) {
			...
			// Zoom into the location
			[self.fromMap setRegion:place.coordinateRegion animated:TRUE];
...
- (IBAction)saveButton:(id)sender {
    NSString* from = self.searchFrom.text;    
    ...
    viewControllerA.fromTxt.text = from;
